...
public interface ProductReportDao extends JpaRepository<ProductReport, UUID> {
    ...
    @Query("select p from ProductReport p where p.product.id = :productID and p.reporter.id = :userID")
    Optional<ProductReport> getProductReport(@Param("productID") UUID productID,@Param("userID") UUID userID);
}
